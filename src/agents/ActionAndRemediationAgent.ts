...
export interface ActionResult {
  ...
  generatedFixScript?: string;    // The actual raw script generated by the LLM
}
...
export class ActionAndRemediationAgent {
  ...
  public async executeActions(input: ActionInput): Promise<ActionResult> {
    ...
    if (primaryRootCause && primaryRootCause.confidence > 0.6) {
      ...
      // ALWAYS generate the fix script so the human can review it in the UI before approval!
      const fixScriptRequirement = `Generate a Bash or Python script to perform the remediation: "${proposedRemediation}". The script must be production-ready and safe. Return ONLY the raw code block.`;
      generatedFixScript = await this.llmClient.generateCode(fixScriptRequirement, 'bash');
      ...
      if (requiresHumanApproval) {
        ...
        executionResults.push(`Auto-executed fix script.`);
